# Convert a Recovery Services Vault Backup Property from GRS to LRS
# 
# WARNING: This is a destructive process. ALL BACKUP DATA WILL BE DELETED!
#
# The script follows these steps:
#    Search for all Recovery Service Vaults in one or multiple subscriptions
#    User selection of Recovery Service Vault across searched subscriptions
#    Disables protection for all types of Backup Items
#    Deletes all Backup Items from the vault
#    Deletes the vault
#
...
# 2. Allow user selection of Subscription for Recovery Service Vault search
...
# 3. Cycle through all Azure subscriptions and display the Recovery Service Vaults.
try {
...
# 5. User selects which Recovery Service Vault to convert to LRS
Write-verbose -Message "Recovery Service Vaults"
$RecoveryServiceVault = $RecoveryServiceVaults |  Out-GridView -Title "Select the Recovery Service Vault to convert to LRS" -OutputMode Single


# 6. Prepare working variables
...
# 7. Set context to the selected RSV for deletion
...
# 9. Rollback any soft deleted items pending for deletion
...
# 10. Invoking API to disable Security features (Enhanced Security) to remove MARS/MAB/DPM servers.
...
# 11. Fetch all protected items and servers, and force dissable & delete them
Write-Verbose -Message "Fetching all protected items and servers, force dissabling and deleting them. Please wait..."
...
# 14. Recheck presence of backup items in vault
...
# 15. Display items which are still present in the vault and might be preventing vault deletion.
...
# 16. Set RSV Backup Property to LocallyRedundant storage (LRS)
